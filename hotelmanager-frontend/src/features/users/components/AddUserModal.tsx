...
const AddUserModal: React.FC<AddUserModalProps> = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    if (firstName && lastName) {
      const substitutedFirst = substituteLetters(
        firstName.substring(0, 1).toLowerCase()
      );
      const substitutedLast = substituteLetters(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError("Unable to add the user.");
    } finally {
      setLoading(false);
    }
  };
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <input
            name="email"
            type="email"
            placeholder="Email automatically generated"
            value={form.email}
            readOnly
