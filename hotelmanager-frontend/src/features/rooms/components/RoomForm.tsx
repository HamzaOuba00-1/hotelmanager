...
const ROOM_STATES = [
  ...
  { value: "RESERVEE", label: "Reserved" },
  { value: "CHECKIN", label: "Check-in" },
  { value: "ROOM_SERVICE", label: "Room service / maintenance" },
  { value: "CHECKOUT", label: "Check-out" },
  { value: "A_VALIDER_LIBRE", label: "To validate (available)" },
  { value: "A_NETTOYER", label: "To be cleaned" },
  { value: "EN_NETTOYAGE", label: "Cleaning in progress" },
  { value: "A_VALIDER_CLEAN", label: "To validate (clean)" },
];
...
export default function RoomFormPremium({
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    if (!Number.isInteger(roomNumber) || roomNumber <= 0) {
      setLoading(false);
      setErrMsg("Room number must be a positive integer.");
      return;
    }

    if (!Number.isInteger(floor) || floor < 0) {
      setLoading(false);
      setErrMsg("Floor must be an integer greater than or equal to 0.");
      return;
    }
    ...
    try {
      ...
      if (status === 409) {
        setErrMsg(
          pd?.detail || "This room number already exists for this hotel."
        );
      } else if (status === 400) {
        if (pd?.errors) setFieldErrors(pd.errors);
        setErrMsg(pd?.detail || pd?.title || "Invalid request.");
      } else if (pd?.title || pd?.detail) {
        setErrMsg(
          `${pd.title ?? "Error"}${pd.detail ? `: ${pd.detail}` : ""}`
        );
      } else {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <label className="input-label">
          ...
          <input
            ...
            className={`input-premium ${
              fieldErrors["roomNumber"] ? "border-red-300" : ""
            }`}
          />
        ...
        <label className="input-label">
          ...
          <input
            ...
            required
            className={`input-premium ${
              fieldErrors["floor"] ? "border-red-300" : ""
            }`}
          />
          {fieldErrors["floor"] && (
            <p className="mt-1 text-xs text-red-600">
        ...
        <label className="input-label sm:col-span-2">
          ...
          <textarea
            ...
            onChange={handleChange}
            placeholder="Details, amenities, view..."
            className="input-premium"
          />
        </label>
      </div>
