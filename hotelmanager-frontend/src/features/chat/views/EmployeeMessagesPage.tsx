...
const EmployeeChatPage: React.FC = () => {
  ...
  const [channelMembersCache, setChannelMembersCache] = useState<
    Map<number, MemberLite[]>
  >(new Map());
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setActive((a) =>
        a
          ? data.find((d) => d.id === a.id) ?? data[0] ?? null
          : data[0] ?? null
      );
    } finally {
  ...
  useEffect(() => {
    (async () => {
      if (!selectedCrew || selectedCrew.type !== "CREW") {
        setCrewMembers([]);
        return;
      }
  ...
  const displaySender = (m: ChatMessage) => {
    const full = joinName(
      (m as any).senderFirstName,
      (m as any).senderLastName
    );
    if (full) return full;
    const sid = (m as any).senderId as number | undefined;
    if (sid && usersById.has(sid)) {
      const u = usersById.get(sid)!;
      return joinName(u.firstName, u.lastName) || u.email || "User";
    }
    return "User";
  };
  ...
  const pickerMembers = useMemo(() => {
    const base: MemberLite[] =
      pickerMode === "channel"
        ? active
          ? channelMembersCache.get(active.id) ?? []
          : []
        : crewMembers;
  ...
  const createDM = async () => {
    ...
    try {
      ...
      showToast(e?.response?.data?.detail || "Cannot create conversation");
    } finally {
      setCreatingDM(false);
    }
  };
    ...
    <div className="p-6 space-y-6">
        ...
        {myCrewChannels.length > 0 && (
          <div className="w-full max-w-5xl flex flex-col items-center gap-2">
            <div className="text-xs text-gray-500">
              My crew: <b>{selectedCrew?.name ?? myCrewChannels[0]?.name}</b>
            </div>
            ...
            {!!crewMembers.length && (
              <div className="flex items-center gap-2 flex-wrap justify-center">
                ...
                <button
                  ...
                  title="Pick among active channel members"
                >
                  <Plus className="w-3.5 h-3.5" />
                  Contact a channel member
                </button>
              </div>
            )}
          </div>
            ...
            <>
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-gray-500">
                      ...
                      {active.type === "ANNOUNCEMENT" ? " • read-only" : ""}
                    </div>
                  </div>
                </div>
                ...
                <button
                  ...
                  onClick={openChannelPicker}
                  title="Contact a channel member"
                >
              ...
              <div className="mt-4 flex items-center gap-2">
                <input
                  ...
                  placeholder={
                    canWrite(active)
                      ? "Write a message… (Ctrl/Cmd+Enter)"
                      : "Read-only channel"
                  }
                  value={draft}
                ...
                <button
                  ...
                  onClick={onSend}
                  disabled={!canWrite(active) || !draft.trim()}
                >
      ...
      {dmPickerOpen && (
        <div
          ...
          onClick={(e) =>
            e.currentTarget === e.target && setDmPickerOpen(false)
          }
        >
          <div
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-800">
                {pickerMode === "crew"
                  ? "New conversation (my crew)"
                  : "New conversation (channel members)"}
              </h3>
              <button
            ...
            <div className="mb-3 text-sm text-gray-600">
              {pickerMode === "crew"
                ? "Choose a member from your crew."
                : "Choose a member who belongs to the active channel."}
            </div>
              ...
              {pickerMembers.map((m) => {
                ...
                return (
                  <button
                    key={m.id}
                    onClick={() => setDmPick(m)}
                    className={`w-full flex items-center justify-between rounded-xl border px-4 py-2 text-left ${
                      selected
