...
import {Cog, LayoutDashboard, Search, Users} from "lucide-react";
...
const Sidebar: React.FC<{ logoSrc?: string }> = ({logoSrc}) => (
    <aside className="w-64 shrink-0 bg-[#F6F8F7] h-screen p-6 flex flex-col">
        ...
        <nav className="space-y-1 flex-1">
            ...
            <SidebarLink to="/dashboard/manager/rooms" icon={<span role="img"
                                                                   aria-label="Bed">🛏</span>}> {/* Or use an icon from lucide-react if available */}
                Chambres
            </SidebarLink>

            <SidebarLink to="/dashboard/manager/chat" icon={<span role="img" aria-label="Chat">💬</span>}>
                Messagerie
            </SidebarLink>
