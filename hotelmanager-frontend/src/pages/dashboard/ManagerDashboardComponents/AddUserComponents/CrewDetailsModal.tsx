...
// ✅ base carte neutre (sans fond de sélection)
const cardBase =
...
const btnGhost = btnBase + " border border-gray-200 text-gray-700 hover:bg-gray-100";
const btnDanger = btnBase + " border border-red-500 text-red-500 hover:bg-red-50";
const btnPrimary =
  btnBase + " text-white bg-gradient-to-br from-emerald-600 to-emerald-500 hover:shadow-lg";
  ...
  return (
    ...
    >
      <div
        ...
        className={clsx(
          "w-full max-w-3xl rounded-3xl border border-white/20 shadow-2xl",
          // ✅ empêche la modal d’être gigantesque
      ...
      >
        ...
        <div
          className={clsx(
            "bg-white/70 backdrop-blur-xl p-6 sm:p-8",
            // ✅ scroll global si jamais la modal est chargée
            "overflow-y-auto"
          )}
          style={{ maxHeight: "calc(85vh - 110px)" }} // header approx
        >
              ...
              <div>
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm text-gray-600">
                    {editMode ? "Sélectionner des membres" : "Membres"}
                  </span>
                  {!editMode && (
                    <span className="text-xs text-gray-500">
                      {crew.memberCount} membres
                    </span>
                  )}
                  {editMode && (
                    <span className="text-xs text-gray-500">
                ...
                {/* ✅ Zone scroll dédiée membres (anti hauteur énorme + anti crop) */}
                <div className="max-h-[360px] overflow-y-auto px-2 py-2">
                  {!editMode ? (
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                      {crew.members.map((m) => (
                        <div key={m.id} className={clsx(cardBase, "bg-white/60 border-white/30 m-0.5")}>
      ...
      <ConfirmModal
        ...
        message={
          <div className="space-y-1">
            <div>
              Nom&nbsp;: <span className="font-medium">{name || "(vide)"}</span>
            </div>
            <div>
